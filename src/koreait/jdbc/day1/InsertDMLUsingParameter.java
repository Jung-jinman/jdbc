...
public class InsertDMLUsingParameter {
	...
	public static void main(String[] args) {
		...
		try (
			...
			// PreparedStatemont 객체를 생성하면서 실행할 sql을 설정합니다.
			// PreparedStatemont 객체는 Connection 객체 메소드를 만듭니다. Connection 구현객체는 dbms 종류에 따라
			// 생성되고 PreparedStatemont 객체도 그에 따라 구현 객체가 결정됩니다.
			PreparedStatement pstmt = conn.prepareStatement(sql);
			
			// 'PreparedStatement() 메소드는 객체를 생성'해서 리턴합니다.
			
			// 매개변수에 값 전달하기 : 매개변수의 데이터형식 순서 문자열, 문자열, 정수, 문자열
			// setXXXX 메소드의 첫 번째 인자는 매개변수의 인덱스, 두 번째 인자는 값입니다. XXXX는 매개변수의 타입
			pstmt.setString(1, "2023003");		// 1번 인덱스에 "2023003" 대입.
			pstmt.setString(2, "김땡사");
			pstmt.setInt(3, 17);
			pstmt.setString(4, "강원도");
			
			pstmt.execute();			// PreparedStatemont 객체로 execute 하면 'SQL이 실행'됩니다.
			pstmt.close();
 ...
 * 특징은 sql을 먼저 컴파일 하고 sql 실행에 필요한 값은 실행할 때 매개변수로 전달하는 방식입니다.
 */
